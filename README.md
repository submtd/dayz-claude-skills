...
| `dayz-deploy` | Putting a Nitrado mission folder under GitHub and shipping it with the Release-triggered FTP deploy: bootstrap from a live server, the everyday tag-and-release flow, rolling back a change that broke the server, and why the deploy's state file is not the server — hand uploads no deploy removes, hand deletes no deploy resends, bot-written files, and Nitrado's mission-reset and storage-wipe toggles. `audit.py` finds tags that never deployed and lints the workflow; `nitrado.py` compares the live server with the repo through the Nitrado API, read-only. |
...
python3 skills/dayz-deploy/scripts/nitrado.py drift /path/to/mission-repo   # needs NITRADO_TOKEN
```
